...
def max_pool_2x2(x):
    # 池化卷积结果（conv2d）池化层采用kernel大小为11*11，步数也为5，周围补0，取最大值。数据量缩小了4倍
    return tf.nn.max_pool(x, ksize=[1, 11, 11, 1], strides=[1, 6, 6, 1], padding='SAME')

# 二，定义输入输出结构

# 声明一个占位符，None表示输入图片的数量不定，101*99图片分辨率
xs = tf.placeholder(tf.float32, [None, 101 * 99])
# 类别是2个类别，对应输出分类结果
